...
class PromptConstructor:
    ...
    def build_generation_prompt_for_fields(self, company_name: str, field_names: List[str]) -> str:
        ...
        field_constraints = self._constraints_for_fields(field_names)
...
8. Respect each field constraint block exactly (data type, regex, nullability, business rules)

=== ALL REQUIRED FIELDS ===
{json.dumps(json_template, indent=2)}

=== FIELD CONSTRAINTS (AUTHORITATIVE) ===
...
"""
    ...
    def build_repair_prompt_for_fields(
        ...
        field_constraints = self._constraints_for_fields(failed_fields)
...
Field constraints for these keys:
{json.dumps(field_constraints, indent=2)}
...
"""
    ...
    def _constraints_for_fields(self, field_names: List[str]) -> Dict[str, Dict[str, Any]]:
        """Return compact constraint metadata for prompt guidance."""
        out: Dict[str, Dict[str, Any]] = {}
        for field in field_names:
            row = self.schema_loader.get_field(field) or {}
            out[field] = {
                "type_hint": row.get("update_frequency") or row.get("data_type"),
    ...
    def build_consolidation_prompt(
        ...
        required_keys = json.dumps(canonical_fields, ensure_ascii=True)

        return (
            f"You are an expert data reconciliation specialist. Consolidate {model_count} AI model outputs "
            f"into one authoritative {len(canonical_fields)}-field dataset for {company_name}.\n\n"
            "Rules:\n"
            ...
            "6) Do NOT wrap with markdown/code fences\n"
            "7) Every required key must appear exactly once\n\n"
            f"Required keys (exact list): {required_keys}\n\n"
            "Field candidates (field -> provider -> value):\n"
            f"{compact_candidates}\n\n"
            f"Return ONLY the final consolidated JSON object with all {len(canonical_fields)} fields."
        )
